    ...
    *,
    ...
    if dry_run:
        for index, card in enumerate(collected, start=1):
            ...
            skipped_count += 1
            items.append(_dry_run_item(index, card))
    else:
        async with LinkedInBrowserSession() as browser_session:
            ...
            for index, card in enumerate(collected, start=1):
                ...
                try:
                    ...
                    ok = response.status not in {"error", "rejected"} and response.job_id is not None
                    if ok:
                        success_count += 1
                    else:
                        failed_count += 1
...
def _dry_run_item(index: int, card: dict) -> dict:
    return {
        "row_number": index,
        "linkedin_job_id": card.get("linkedin_job_id"),
